...
import {
  ...
  identityMatrix,
  scaleMatrix,
} from "@visx/zoom/lib/util/matrix";
import {
  TransformMatrix,
  Point,
  Translate,
...
export type ZoomProps<ElementType> = {
  ...
  transformMatrixSetter: (mat: TransformMatrix) => void;
...
function ZustandZoom<ElementType extends Element>({
  scaleXMin = 0,
  scaleXMax = Infinity,
  scaleYMin = 0,
  scaleYMax = Infinity,
  ...
  const matrixStateRef = useRef<TransformMatrix>(transformMatrix);
  ...
  const setTransformMatrix = useCallback(
    (newTransformMatrix: TransformMatrix) => {
      ...
      // });
      matrixStateRef.current = newTransformMatrix;
      transformMatrixSetter(newTransformMatrix);
    },
    [defaultConstrain]
  );
  ...
  const scale = useCallback(
    ({ scaleX, scaleY: maybeScaleY, point }: ScaleSignature) => {
      ...
      const translate = applyInverseMatrixToPoint(
